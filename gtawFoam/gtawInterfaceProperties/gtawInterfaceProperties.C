...
void Foam::gtawInterfaceProperties::correctContactAngle
(
...
void Foam::gtawInterfaceProperties::calculateK()
{
...
Foam::gtawInterfaceProperties::sigmaT() const
{
...
//    return fvc::interpolate(sigmaK() * (1.0 - mixture_.alpha3()))*fvc::snGrad(mixture_.alpha1());
  
    /*
    // Only for v. specific cases! Don't use
    return 
    (
        sigma12_*fvc::interpolate(-fvc::div(nHatfvMetalCorr & mesh.Sf()))*
        (
            fvc::interpolate(alphaMetal)*fvc::snGrad(mixture_.alpha1())
            - fvc::interpolate(mixture_.alpha1())*fvc::snGrad(alphaMetal)
        )
    );
    */
...
Foam::tmp<Foam::surfaceScalarField> 
Foam::gtawInterfaceProperties::nHatfMetal() const
{
    const fvMesh& mesh = mixture_.alpha1().mesh();
    const volScalarField alphaMetal = alphaMetal_;
...
Foam::gtawInterfaceProperties::nHatfvMetal() const
{
    const volScalarField alphaMetal = alphaMetal_;
...
Foam::tmp<Foam::surfaceScalarField> 
Foam::gtawInterfaceProperties::nHatMetalMG() const
{
    const volScalarField alphaMetal = alphaMetal_;
